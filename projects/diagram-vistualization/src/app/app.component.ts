...
export class AppComponent implements OnInit {
  ...
  public nodes: NodeModel[] = [
    ...
    {
      id: 'hazardEnd', width: 40, height: 40, offsetX: 305, offsetY: 370,
      shape: {
        type: 'Bpmn', shape: 'Event',
        event: { event: 'End' },
      },
    ...
    {
      id: 'cancelledEnd', width: 40, height: 40, offsetX: 545, offsetY: 370,
      shape: {
        type: 'Bpmn', shape: 'Event',
        event: { event: 'End' },
      },
    ...
    {
      id: 'end', width: 40, height: 40, offsetX: 665, offsetY: 180,
      shape: {
        type: 'Bpmn', shape: 'Event',
        event: { event: 'End' }
      },
    ...
    {
      id: 'processesStart', width: 30, height: 30,
      shape: {
        type: 'Bpmn', shape: 'Event',
        event: { event: 'Start' }
    ...
    {
      id: 'processesEnd', width: 30, height: 30,
      shape: {
        type: 'Bpmn', shape: 'Event',
        event: { event: 'End' }
      },
  ...
  public contextMenuClick(args: MenuEventArgs): void {
    if (this.diagram.selectedItems.nodes.length > 0) {
      const bpmnShape: BpmnShapeModel = this.diagram.selectedItems.nodes[0].shape as BpmnShapeModel;
      ...
      if (args.item.iconCss.indexOf('e-loop') > -1) {
        const loop: string = (args.item.id === 'LoopNone' as BpmnLoops) ? 'None' : args.item.id;
      ...
      if (args.item.iconCss.indexOf('e-compensation') > -1) {
        const compensation: boolean = (args.item.id === 'CompensationNone') ? false : true;
        if (bpmnShape.activity.activity === 'Task') {
      ...
      if (args.item.iconCss.indexOf('e-call') > -1) {
        const compensation: boolean = (args.item.id === 'CallNone') ? false : true;
        if (bpmnShape.activity.activity === 'Task') {
      ...
      if (args.item.iconCss.indexOf('e-data') > -1) {
        const call: string = args.item.id === 'DataObjectNone' ? 'None' : args.item.id;
        bpmnShape.dataObject.type = call as BpmnDataObjects;
      }
      if (args.item.iconCss.indexOf('e-collection') > -1) {
        const call: boolean = (args.item.id === 'Collectioncollection') ? true : false;
        bpmnShape.dataObject.collection = call;
      }
      if (args.item.iconCss.indexOf('e-task') > -1) {
        const task: string = args.item.id === 'TaskNone' ? 'None' : args.item.id;
        if (bpmnShape.activity.activity === 'Task') {
